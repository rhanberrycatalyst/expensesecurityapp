...
public class ReportDaoHibernate implements ReportDao {
	...
	/**
	 * Creates a new row in the Report table in the database and updates EndUser
	 * and Project tables. LineItem feature then loops through LineItems and
	 * adds new rows to Line Item tables in database.
	 * 
	 * @param report
	 */
	@Override
	public void add(Report report) {
			...
			ReportStatus reportStatus = em
					.createQuery("SELECT s FROM ReportStatus s WHERE s.reportStatusId = 1", ReportStatus.class)
					.getSingleResult();
